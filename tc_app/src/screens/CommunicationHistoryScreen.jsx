...
import {
   StyleSheet, Text, View, TouchableOpacity, Alert, FlatList, TextInput, Button,ScrollView,RefreshControl,BackHandler ,AppState,Platform,Linking, Image
} from 'react-native';
    ...
    if (route.previous == 'LogIn'){
      ...
      fetch(domain+'batch_app/api_system_app.php?'+Date.now(),
      {
        ...
        .then((json) => {
          ...
          // ローカルDB用定型文
          const fixed_mst_data = Object.entries(json.fixed_array).map(([key, value]) => {
            return ({key, value})
          });
          ...
          db.transaction((tx) => {
            ...
            tx.executeSql(
              ...
              (_, { rows }) => {
                ...
                tx.executeSql(
                  `select * from customer_mst where customer_id in (`+cus_offline_list_id+`) order by time desc;`,
                  [],
                  (_, { rows }) => {
                ...
                const errMsg = '端末に保存された'+cus_count+'件のメッセージのみ表示します';
        ...
        tx.executeSql(
          ...
          (_, { rows }) => {
            ...
            if (rows._array.length) {
              ...
              // 削除
              const del_staff_list = local_staff_list.filter(c => server_staff_list.indexOf(c) == -1)
        ...
        tx.executeSql(
          ...
          (_, { rows }) => {
            ...
            if (!rows._array.length) {
              ...
              db.transaction((tx) => {
                Promise.all(customer.map((cus) => {
                  
                  if (cus.html_flg || (cus.communication_title === '入居申込書' && cus.communication_status === 'その他')) {
                    cus.communication_note = cus.communication_note.replace(/<("[^"]*"|'[^']*'|[^'">])*>/g,'');
                  }
                  ...
                  if (cus.category_number != '1') {
                    tx.executeSql(
                      ...
                      [cus.customer_user_id,cus.name,cus.kana,cus.communication_time,cus.communication_title,cus.line_note?cus.line_note:cus.communication_note,cus.mail1,cus.mail2,cus.mail3,cus.line,cus.staff_name,cus.media,cus.article_url,cus.reverberation_user_id,cus.coming_user_id,cus.coming_day1,status],
                      () => {
            ...
            if (rows._array.length) {
              ...
              rocal.map((r) => {
                customer.map((cus) => {
                  if (cus.customer_user_id === r.customer_id && cus.category_number != '1') {
                    
                    if (cus.communication_time != r.time || cus.communication_note != r.note|| cus.name != r.name) {
                      ...
                      db.transaction((tx) => {
                        ...
                        tx.executeSql(
                          ...
                          [cus.name,cus.kana,cus.communication_time,cus.communication_title,cus.line_note?cus.line_note:cus.communication_note,cus.mail1,cus.mail2,cus.mail3,cus.line,cus.staff_name,cus.media,cus.article_url,status,cus.customer_user_id],
                          () => {
                            tx.executeSql(
                              `select * from customer_mst order by time desc;`,
                              [],
              ...
              customer.map((cus) => {
                add_customer.map((add) => {
                  ...
                  if (cus.customer_user_id === add && cus.category_number != '1') {
              ...
              // 削除
              const del_customer = local_cus_id.filter(c => server_cus_id.indexOf(c) == -1)
        ...
        tx.executeSql(
          ...
          (_, { rows }) => {
            ...
            if (rows._array.length) {
                ...
                fixed.map((f) => {
                  if (f.fixed_id && r.fixed_id) {
                    if (f.fixed_id === r.fixed_id) {
                      ...
                      if (f.category != r.category || f.title != r.title || f.mail_title != r.mail_title || f.note != r.note) {
                        
                        db.transaction((tx) => {
                        
                          tx.executeSql(
                            `update fixed_mst set category = (?), title = (?), mail_title = (?), note = (?) where ( fixed_id = ? );`,
                            [f.category,f.title,f.mail_title,f.note,f.fixed_id],
                            () => {
                              tx.executeSql(
                                `select * from fixed_mst;`,
                                [],
              ...
              // 削除
              const del_fixed_list = local_fixed_list.filter(c => server_fixed_list.indexOf(c) == -1)
    ...
    headerRight: () => (
      <View style={{flexDirection:'row',justifyContent: 'center'}}>
        ...
        <View style={bell_count?styles.belltext:{display:'none'}}>
          <Text Id="bell_text" style={{color: 'white'}} >{bell_count}</Text>
        </View>
        </View>
        <View>
        <Feather
              ...
              >
                <View style={styles.ListInner}>
                  <View style={{flexDirection: 'row'}}>
                    <Text style={item.status == '未対応'?{color:'red',fontSize:18}:{display:'none'}}>●</Text>
                  ...
                  <Text style={styles.date}>{item.time?item.time.slice(0,-3):''}</Text>
                  <Text style={styles.message} numberOfLines={1}>
                    {(item.title==='スタンプ'?'スタンプを送信しました':!item.note?item.title:item.note)}
                  </Text>
                </View>
              </TouchableOpacity>
            )
