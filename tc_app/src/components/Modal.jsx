...
export function MyModal0(props) {
  const { isVisible, onSwipeComplete, onPress, send_image, pickDocument } =
    props;
...
export function MyModal1(props) {
  ...
  useEffect(() => {
    ...
    if (mail_set) {
      ...
      if (mail_set.brower_mail) {
        ...
        setShop_Value(
          mail_set.mail_select ? mail_set.mail_select : shop_mail[0]
        );
    ...
    Alert.alert("通話画面を開きますか？", "", [
      {
        ...
        onPress: () => {
          fetch(domain + "video/?customer_id=" + id, {
            ...
            .then((json) => {
              Linking.openURL(json).catch((err) => {
                console.log(err);
  ...
  const onSubmit = () => {
    ...
    if (note) {
      if (!mail_subject) {
        Alert.alert("件名が入っていませんがよろしいですか？", "", [
          {
            text: "はい",
            onPress: () => {
              props.onSend(
                [
                  ...
                  [
                    ...
                    mail_subject,
                    isEnabled || checked,
                    isEnabled || checked ? formatDate(date) : "",
                    res_id,
                    "",
                    true,
                    filedata,
    ...
    if (Platform.OS === "ios") {
      ...
      return (
        <View>
          ...
          <View
            style={[checked === true ? "" : { display: "none" }, styles.input]}
          >
  ...
  return (
    ...
    >
      ...
      <View style={styles.sydemenu}>
        <TouchableOpacity style={styles.menucircle} onPress={openProperty}>
          <Feather name="home" color="#1f2d53" size={28} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.menucircle} onPress={openFixed}>
          <Feather name="file-text" color="#1f2d53" size={28} />
        </TouchableOpacity>
        <TouchableOpacity
          style={option ? styles.menucircle : { display: "none" }}
          onPress={() => {
            openOnline_call(route.customer);
          }}
        >
      ...
      >
        ...
        >
          ...
          <View style={styles.form}>
            <ScrollView style={{ height: Platform.OS === "ios" ? 400 : 500 }}>
              {rrr()}
              ...
              >
                <TouchableOpacity
                  onPress={img_Delete}
                  style={
                    filename ? { marginHorizontal: 5 } : { display: "none" }
                  }
                >
              ...
              <Text style={styles.inlabel}>
                ※携帯電話に送る際は2MB以下にしてください
              </Text>
...
export function MyModal3(props) {
  ...
  if (stations) {
    var get_station = stations.map((value) => {
      return {
        ...
        name: value.station_name + "(" + value.line_name + ")",
      };
    });
  }
  ...
  useEffect(() => {
    if (c_d) {
      ...
      setValue_Constructure(
        c_d.constructure ? c_d.constructure.split(",") : null
      );
  ...
  const proInsert = (item) => {
    var msg =
      ...
      "㎡" +
      "\n\n" +
      "https://www.total-cloud.net/show/" +
      route.customer +
      "/1/" +
      item.article_id +
      "/" +
      ...
      <View style={[{ height: 400 }, styles.template]}>
        ...
        >
          <View style={[{ height: 600 }, styles.modalInner]}>
            <TouchableOpacity style={styles.close} onPress={recommended}>
              <Feather name="x-circle" color="gray" size={35} />
            </TouchableOpacity>
            <View style={styles.form}>
              <ScrollView style={{ height: 400 }}>
                <View>
                  ...
                  <Text style={styles.label}>
                    間取り
                    <Text style={styles.inlabel}>　※複数選択可</Text>
                  </Text>
                  <DropDownPicker
                    placeholder="------------"
                  ...
                  <View style={detail === true ? "" : { display: "none" }}>
                    ...
                    >
                      <Text style={[styles.label, { width: "20%" }]}>
                        コンロ：
                      </Text>
                      <DropDownPicker
                        placeholder="------------"
                    ...
                    <View
                      style={[
                        styles.input,
                        open_direction || open_setubi || open_stove === true
                          ? { marginBottom: 120 }
                          : "",
              ...
              <View style={{ flexDirection: "row", alignSelf: "center" }}>
                <TouchableOpacity
                  onPress={onDelete}
                  style={[styles.draft, { marginTop: 0 }]}
                >
                  <Text>リセット</Text>
                </TouchableOpacity>
                <TouchableOpacity
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity activeOpacity={1} style={styles.property}>
              <View style={styles.propertyInner}>
                <Text style={styles.propertyTitle}>
                  {item.article_name}
                  {"\n"}
                  <Text style={{ fontSize: 12 }}>{item.floor}階</Text>
                </Text>
                <View style={styles.propertyInfo}>
                  <Text>沿線：</Text>
                  <Text>{item.line_name1}</Text>
                </View>
                <View style={styles.propertyInfo}>
                  <Text>駅名：</Text>
                  <Text>{item.station_name1}駅</Text>
                </View>
                <View style={styles.propertyInfo}>
                  <Image
                  ...
                  <View>
                    <Text>
                      <Text style={{ color: "red" }}>{item.rent / 10000}</Text>
                      万({item.general}円)
                    </Text>
                    <Text>徒歩{item.station_time1}分</Text>
                    <Text>
                      {item.layout}/{item.category}
                    </Text>
...
export function MyModal5(props) {
  ...
  useEffect(() => {
          ...
          if (cus.reverberation.article_id || cus.reverberation.private_id) {
            fetch(domain + "batch_app/api_system_app.php?" + Date.now(), {
              ...
              .then((json) => {
                if (!json) {
                  setText(
                    "こちらの反響は、問い合わせ物件の情報がTCにないため自動追客できません\n手動対応お願い致します"
                  );
                  setPattern(2);
                } else {
    ...
    if (cus) {
      ...
      if (
        cus.reverberation.media == "SUUMO" &&
        !cus.main.tel1 &&
        !cus.main.tel2 &&
        cus.main.tel3.substring(0, 3) == "050"
      ) {
    ...
    fetch(domain + "batch_app/api_system_app.php?" + Date.now(), {
      ...
      .then((json) => {
        if (json.follow_data_all) {
          const f_d = Object.entries(json.follow_data_all).map(
            ([key, value]) => ({ key, value })
          );
      ...
      return (
        <>
          <ScrollView style={{ height: 280 }}>
            ...
            <Text style={styles.cus_contents}>{name}</Text>
            <Text style={styles.cus_label}>【TEL】</Text>
            <Text style={styles.cus_contents}>{tel}</Text>
            <Text
              style={
                suumo
                  ? [styles.cus_contents, { color: "red" }]
                  : { display: "none" }
              }
            >
              ※こちらは有効期限付きの番号です。{"\n"}
            ...
            <Text style={styles.cus_contents}>{title}</Text>
            <Text style={styles.cus_label}>【本文】</Text>
            <Text style={styles.cus_contents}>{note}</Text>
            <View style={{ flexDirection: "row" }}>
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              <Text
                style={
                  contact_tel == "TEL電話なし" || contact_tel == "応答なし"
                    ? [styles.cus_label, { color: "red" }]
                    : { display: "none" }
                }
              >
                ※必須
            ...
            <TextInput
              ...
              numberOfLines={11}
              editable={
                contact_tel != "電話で会話した" && free_radio != "無し"
                  ? true
                  : false
              }
            />
            ...
            <View style={styles.follow_item}>
              <Text style={[styles.cus_label, { marginHorizontal: 20 }]}>
                希望条件1
              </Text>
              <TouchableOpacity
                onPress={() => {
              ...
              <TouchableOpacity
                onPress={() => {
                  follow_check(follow_item1);
                }}
                style={
                  follow_item1 ? styles.follow_item_btn : { display: "none" }
                }
              >
                <Text>確認</Text>
              </TouchableOpacity>
            ...
            <View style={styles.follow_item}>
              ...
              <TouchableOpacity
                onPress={() => {
                  follow_check(follow_item2);
                }}
                style={
                  follow_item2 ? styles.follow_item_btn : { display: "none" }
                }
              >
                <Text>確認</Text>
              </TouchableOpacity>
            ...
            <View style={styles.follow_item}>
              ...
              <TouchableOpacity
                onPress={() => {
                  follow_check(follow_item3);
                }}
                style={
                  follow_item3 ? styles.follow_item_btn : { display: "none" }
                }
              >
                <Text>確認</Text>
              </TouchableOpacity>
            ...
            <View style={styles.follow_item}>
              ...
              <TouchableOpacity
                onPress={() => {
                  follow_check(follow_item4);
                }}
                style={
                  follow_item4 ? styles.follow_item_btn : { display: "none" }
                }
              >
                <Text>確認</Text>
              </TouchableOpacity>
    ...
    >
      ...
      <View style={[{ height: 500 }, styles.template]}>
        ...
        <DropDownPicker
          ...
          zIndex={999}
        />
        <View style={{ marginTop: 10 }}>{setView(cus, pattern)}</View>
...
export function MyModal5_condition(props) {
  ...
  const [constructure, setConstructure] = useState([
    ...
    {
      label: "PC(プレキャストコンクリート)",
      value: "PC(プレキャストコンクリート)",
    },
    {
      label: "HPC(鉄骨プレキャストコンクリート)",
      value: "HPC(鉄骨プレキャストコンクリート)",
    },
      ...
      fetch(domain + "batch_app/api_system_app.php?" + Date.now(), {
        ...
        .then((json) => {
          ...
          if (json.shop_choice.line_list) {
            fetch(
              domain + "js/data/reins_" + json.shop_choice.shop_id + ".json"
            )
              .then((response) => response.json())
              .then((json) => {
                setS_list(json);
          ...
          if (json.shop_choice.line_list) {
            fetch(
              domain + "js/data/address_" + json.shop_choice.shop_id + ".json"
            )
              .then((response) => response.json())
              .then((json) => {
                setA_list(json);
          ...
          if (json.follow_data) {
            ...
            setValue_Constructure(
              fd.constructure ? fd.constructure.split(",") : null
            );
  ...
  const get_article_cnt = () => {
    ...
    formData.append(
      "val[rent_from]",
      value_rent_from ? `${value_rent_from}` : ""
    );
    ...
    formData.append(
      "val[station_time]",
      value_station_time ? `${value_station_time}` : ""
    );
    formData.append("val[area]", areas ? `${areas}` : "");
    formData.append(
      "val[exclusive_from]",
      value_exclusive_from ? `${value_exclusive_from}` : ""
    );
    formData.append(
      "val[exclusive_to]",
      value_exclusive_to ? `${value_exclusive_to}` : ""
    );
    formData.append("val[category]", value_category ? `${value_category}` : "");
    formData.append(
      "val[constructure]",
      value_constructure ? `${value_constructure}` : ""
    );
    formData.append("val[built]", value_built ? `${value_built}` : "");
    formData.append("val[setubi]", value_setubi ? `${value_setubi}` : "");
    formData.append(
      "val[pet]",
      value_setubi && value_setubi.includes("ペット飼育可") ? 1 : ""
    );
    formData.append(
      "val[order_item]",
      value_order_item ? `${value_order_item}` : ""
    );
    ...
    >
      <View style={[{ height: 650 }, styles.modalInner]}>
        ...
        <View style={styles.form}>
          ...
          <ScrollView style={{ height: 480, position: "relative", bottom: 10 }}>
            <View>
              <Text style={styles.label}>賃料</Text>
              <View
                style={[
  ...
  useEffect(() => {
    ...
    if (overlap && overlap.list) {
      ...
      if (overlap.overlap == "1") {
        ...
        setText(
          `${s[0][1].name_1 + "　" + s[0][1].name_2}` +
            "さんが担当している\nお客様：" +
            `${overlap.list[0].name}` +
            "さんと連絡先が同一です。\nこのお客様にまとめますか？"
        );
      } else if (overlap.overlap == "2") {
        // 重複【既存あり(1件以上)】
        setText(
          "下記お客様たちと同一の連絡先をもっています。\nどれかのお客様にまとめますか？"
        );
      } else if (overlap.overlap == "3") {
        // 重複【新規に既存有り】
  ...
  function setView(num, overlap) {
    ...
    if (num == "1") {
      return (
        <ScrollView style={{ height: 280 }}>
          <Text style={styles.cus_label}>【氏名】</Text>
          <Text style={styles.cus_contents}>{overlap.main.name}</Text>
          <Text style={styles.cus_label}>【件名】</Text>
          <Text style={styles.cus_contents}>{overlap.main.title}</Text>
          <Text style={styles.cus_label}>【メール本文】</Text>
          <Text style={styles.cus_contents}>{overlap.main.note}</Text>
        </ScrollView>
      );
    } else if (num == "2") {
      // スタッフ名
      ...
      const data = overlap.list.map((cus) => {
        return {
          label:
            ...
            cus.name +
            "\n【問い合わせ日】" +
            (cus.inquiry_day ? cus.inquiry_day : "") +
            "\n【来店日】" +
            (cus.coming_day1 ? cus.coming_day1 : ""),
          value: {
            id: cus.customer_id,
    ...
    fetch(domain + "php/ajax/update.php", {
      ...
      .then((json) => {
        if (json) {
          ...
          db.transaction((tx) => {
            tx.executeSql(
              `delete from customer_mst where customer_id in ("` +
                customer_id +
                `") and customer_id != "` +
                user_id +
                `";`,
  ...
  return (
    ...
    >
      ...
      <View style={[{ height: 430 }, styles.template]}>
        ...
        <Text style={styles.templateText}>{text}</Text>
        <View style={{ marginTop: 10 }}>{setView(num, ol)}</View>
        <View style={styles.overlap_btnwrap}>
